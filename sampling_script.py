...
for experiment in experiments:
    ...
    # Save to HDF5
    output_h5_path = os.path.join(script_dir, 'data', 'custom', f'pu1k_05radius_{experiment["name"]}.h5')
...
# # === Worker function to process a single mesh ===
# def process_mesh(mesh_path, experiment):
#     """Process a single mesh according to experiment configuration."""
#     try:
#         result = {
#             'input_patches': [],
#             'gt_patches': []
...
#                 result['input_patches'].append(input_patch.astype(np.float32))
#                 result['gt_patches'].append(gt_patch.astype(np.float32))
...
#             result['input_patches'].append(input_pc.astype(np.float32))
#             result['gt_patches'].append(gt_pc.astype(np.float32))
...
#     os.makedirs(os.path.join(script_dir, 'data', 'custom'), exist_ok=True)
    
#     # Collect mesh files
#     mesh_files = collect_mesh_files()
...
#         # Process meshes in parallel using worker function
#         worker_func = partial(process_mesh, experiment=experiment)
#         results = list(tqdm(pool.imap(worker_func, mesh_files), 
#                           total=len(mesh_files), 
#                           desc=f"Processing {experiment['name']}"))
...
#             input_patches.extend(result['input_patches'])
#             gt_patches.extend(result['gt_patches'])
