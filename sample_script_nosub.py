...
def extract_patches_with_radius(points, patch_size, num_patches, radius_percent=0.2):
...
for experiment in experiments:
    ...
    for mesh_path in tqdm(mesh_files, desc=f"Processing {experiment['name']}"):
        ...
        try:
            ...
            if experiment['patch_based']:
                ...
                # For each ground truth patch, create corresponding input by performing separate sampling from the same region
                for i, gt_patch in enumerate(gt_pc_patches):
                    # We'll sample from the original dense point cloud in the same region as the GT patch
                    # Get the center of the GT patch
                    gt_center = np.mean(gt_patch, axis=0)
                    
                    # Get the radius of the GT patch (approximate)
                    max_dist = np.max(np.linalg.norm(gt_patch - gt_center, axis=1))
                    
                    # Find points in the dense point cloud that are within this region
                    distances = np.linalg.norm(dense_pc - gt_center, axis=1)
                    region_indices = np.where(distances <= max_dist)[0]
                    
                    # If we have enough points in the region, sample from them
                    if len(region_indices) >= input_size:
                        region_points = dense_pc[region_indices]
                        
                        if experiment['input_sampling'] == 'uniform':
                            # Uniform random sampling
                            indices = np.random.choice(len(region_points), input_size, replace=False)
                            input_patch = region_points[indices]
                        elif experiment['input_sampling'] == 'fps':
                            # FPS sampling
                            indices = farthest_point_sampling(region_points, input_size)
                            input_patch = region_points[indices]
                        
                        # Center the input patch at the same center as the GT patch
                        input_center = np.mean(input_patch, axis=0, keepdims=True)
                        input_patch = input_patch - input_center + np.mean(gt_patch, axis=0, keepdims=True)
                    else:
                        # Fallback to the original approach if the region doesn't have enough points
                        print("WE ARE USING FALL BACK!")
                        if experiment['input_sampling'] == 'uniform':
                            # Random subset of GT
    ...
    # Save to HDF5
    output_h5_path = os.path.join(script_dir, 'data', 'custom', f'nosub_pu1k_05radius_{experiment["name"]}.h5')
